@extends('layouts.admin')

@section('title','Status Kunjungan')
@section('page-title','Status Kunjungan Saya')

@section('content')
<div class="card">
  <div class="card-header"><h4>Status Kunjungan</h4></div>
  <div class="card-body">

    @php
      // Fallback: jika variabel belum dikirim, jadikan koleksi kosong agar @forelse aman
      $kunjungan = isset($kunjungan) ? $kunjungan : collect();
    @endphp

    <div class="table-responsive">
      <table class="table table-bordered">
        <thead>
          <tr>
            <th>Pegawai Tujuan</th>
            <th>Keperluan</th>
            <th>Status</th>
            <th>Alasan Penolakan</th>
            <th>Waktu Masuk</th>
            <th>Waktu Keluar</th>
            <th>Aksi</th>
          </tr>
        </thead>
        <tbody>
          @forelse($kunjungan as $k)
            <tr>
              <td>{{ $k->pegawai?->user?->name ?? '-' }}</td>
              <td>{{ $k->keperluan }}</td>
              <td>
                @if($k->status === 'diterima')
                  <span class="badge badge-success">Diterima</span>
                @elseif($k->status === 'ditolak')
                  <span class="badge badge-danger">Ditolak</span>
                @elseif($k->status === 'menunggu')
                  <span class="badge badge-warning">Menunggu</span>
                @elseif($k->status === 'selesai')
                  <span class="badge badge-success">Selesai</span>
                @elseif($k->status === 'sedang_bertamu')
                  <span class="badge badge-warning">Sedang Bertamu</span>
                @else
                  <span class="badge badge-secondary">{{ ucfirst($k->status) }}</span>
                @endif
              </td>
              <td>{{ $k->status === 'ditolak' ? ($k->alasan_penolakan ?? '-') : '-' }}</td>
              <td>{{ $k->waktu_masuk }}</td>
              <td>{{ $k->waktu_keluar ?? '-' }}</td>
              <td>
                @if($k->status === 'sedang_bertamu')
                  <form action="{{ route('tamu.kunjungan.checkout',$k->id) }}" method="POST" style="display:inline">
                    @csrf
                    <button type="submit" class="btn btn-sm btn-danger">
                      <i class="fas fa-sign-out-alt"></i> Checkout
                    </button>
                  </form>
                @elseif($k->status === 'menunggu')
                  <span class="badge badge-secondary"><i class="fa-solid fa-alarm-clock"></i></span>
                @elseif($k->status === 'selesai')
                  <span class="badge badge-success"><i class="fa-solid fa-check"></i></span>
                @else
                  <span class="badge badge-secondary">{{ ucfirst($k->status) }}</span>
                @endif
              </td>
            </tr>
          @empty
            <tr><td colspan="7" class="text-center">Belum ada kunjungan</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
  </div>
</div>
@endsection
