...
@section('content')
    <div class="card card-modern p-4">
        ...
        {{-- ✅ Tampilkan hasil absen setelah redirect (PRG pattern) --}}
        @if (session('hasil_absen'))
            @php $hasil = session('hasil_absen'); @endphp
            @if ($hasil['status'] === 'telat')
                <div class="card card-modern p-4 text-center mt-3 border-warning">
                    ...
                    <p class="mt-2">
                        ...
                        Telat <span class="badge {{ $hasil['telatMenit'] <= 30 ? 'bg-warning' : 'bg-danger' }}">
                            {{ $hasil['telatMenit'] }} menit
                        </span>
                        @if($hasil['telatJamMenit'])
                            (≈ {{ $hasil['telatJamMenit'] }} jam)
                        @endif
                        dari jam 07:30.
                    </p>
                    <div class="alert alert-warning mt-3">Mohon lebih disiplin untuk apel pagi berikutnya.</div>
                </div>
            @else
                <div class="card card-modern p-4 text-center mt-3 border-success">
                    <i class="fa-solid fa-check-circle fa-3x text-success mb-3"></i>
                    <h4 class="text-success">Selamat!</h4>
                    <p class="mt-2">
                        {{ $pegawai->user->name }} (NIP: {{ $pegawai->nip }})<br>
                        Anda masuk tepat waktu pada {{ $hasil['jam'] }}.
                    </p>
                    <div class="alert alert-success mt-3">Kehadiran Anda tercatat dengan baik. Terima kasih atas kedisiplinan Anda.</div>
                </div>
            @endif
        @endif

        @if (!$absen && !session('hasil_absen'))
            {{-- ✅ pakai token, bukan NIP --}}
            <form action="{{ route('apelpagi.masuk', $pegawai->apel_token) }}" method="POST" id="formMasuk">
                @csrf
                <input type="hidden" name="latitude" id="lat">
                <input type="hidden" name="longitude" id="lon">
                <button type="submit" class="btn btn-success btn-lg mt-3" id="btnMasuk">
                    <i class="fa-solid fa-door-open me-2"></i> MASUK
                </button>
            </form>
        @elseif ($absen && !session('hasil_absen'))
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(function(pos) {
                    ...
                    const jarakText = jarak < 1000 ? `${Math.round(jarak)} M` :
          ...
          ✘ Di luar radius kantor
                    ...
                    // Lokasi gagal (HTTP tidak support geolokasi di mobile)
                    // Tidak tampilkan pesan apapun, tombol tetap aktif
                }, {
